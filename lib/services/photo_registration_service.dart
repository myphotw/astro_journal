...
class PhotoRegistrationService {
  ...
  BackendUploadResult? get lastBackendUploadResult => _lastBackendUploadResult;
  ...
  Future<PhotoRegistrationPayload> _prepareFromPick(
    PhotoPickResult picked,
  ) async {
    AppLogger.metadata(
      'PhotoRegistration',
  ...
  Future<ShootingRecord> registerPhotoRecord({
    ...
    final existingRecords = await shootingRecordRepository
        .getByCelestialObjectId(celestialObjectId);

    final hasRepresentative = existingRecords.any(
      (r) => r.isRepresentative && r.photoUri != null && r.photoUri!.isNotEmpty,
    );
  ...
  Future<PhotoSaveData> buildSaveData({
    ...
    return PhotoSaveData(
      ...
      metadataJson:
          payload.exifInfo.makerNoteJson ?? payload.exifInfo.ownerNameJson,
...
class ConfirmedMetadata {
  ...
  factory ConfirmedMetadata.fromExif(ExifInfo exif, {String? targetName}) {
...
enum DuplicateReason { sameFilename, sameCapture }

class DuplicateCheckResult {
  const DuplicateCheckResult({required this.reason, required this.existing});
